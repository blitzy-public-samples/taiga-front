...
class KanbanController extends mixOf(taiga.Controller, taiga.PageMixin, taiga.FiltersMixin, taiga.UsFiltersMixin)
    ...
    @.$inject = [
        ...
        "tgLoader",
        "$timeout",
        # `$inject` maps POSITIONALLY onto the constructor parameters below, so a
        # new entry belongs at the end: inserting it anywhere else silently
        # misbinds every service after the insertion point, with no error raised.
        "tgKanbanReactBridge"
    ]
    ...
    constructor: (@scope, @rootscope, @repo, @confirm, @rs, @rs2, @params, @q, @location,
        ...
        # Built exactly once and never rebuilt. `tgLoadElement` watches this object
        # WITHOUT `objectEquality`, so it compares by identity: rebuilding it on any
        # digest-frequency path would re-assign the host element's DOM properties
        # and re-render every digest. Fresh data reaches the board through the
        # bridge's own accessors instead of through a replacement object.
        #
        # Position matters. It is built at the end of the constructor so it comes
        # after the `applyStoredFilters` early return (that path renders no board)
        # and after the four `defineImmutableProperty` bindings the bridge reads.
    ...
    initializeEventHandlers: ->
        ...
        # `kanban:us:move` is the drag contract, and whoever broadcasts it must send
        # the same six arguments in the same order: it drives both the
        # position-relative order write in `moveUs` and the WIP marker redraw.
        # `moveUs`'s leading `ctx` parameter is the event object `$on` supplies,
        # which is why `moveUsToTop` calls it directly with `null` there.
    ...
    # THIS CONTROLLER IS THE SOLE OWNER OF BOTH
    # KANBAN ROUTING KEYS, and that is an architectural invariant rather than a
    # preference. `app/coffee/modules/events.coffee:214` stores subscriptions in ONE
    # FLAT MAP keyed by routing key for the whole application
    # (`@.subscriptions[routingKey] = subscription`), and `unsubscribe` (`:219-230`)
    # sends a wire command carrying only that key. So a second subscriber to
    # `changes.project.<id>.userstories` or `.projects` would OVERWRITE this
    # controller's callback -- the board would silently stop refreshing -- and either
    # side's cleanup would tear down the shared server-side subscription for both.
    #
    # `app/react/kanban/hooks/useKanbanRealtime.ts` therefore opens NO subscription of
    # its own. It observes the controller's stream instead, through the
    # `events.onAngularEvent` registrar published by
    # `app/coffee/modules/kanban/react-bridge.coffee`, which is why each callback below
    # re-publishes its RAW payload on a scope event. Two properties of that
    # re-publication are load-bearing:
    #
    #   * it carries the message UNTOUCHED and BEFORE this controller's own debounce,
    #     so the React consumer reproduces the incumbent's trailing-edge debounce and
    #     `matches` narrowing itself rather than inheriting a second, stacked wait;
    #   * this controller's own handling is unchanged -- same two keys, same single
    #     `randomTimeout` shared by both, same trailing-only `debounceLeading` wrapper,
    #     same three-value `matches` test, same lightbox deferral. Hoisting each
    #     debounced function into a local is what allows the re-publication to sit
    #     outside the wait without altering the wait.
    #
    # The two event names are mirrored by the exported constants in
    # `app/react/kanban/hooks/useKanbanRealtime.ts`; renaming one end silently stops
    # the board hearing realtime, with no error raised at either end.
    initializeSubscription: ->
        ...
        onUserstoriesMessage = debounceLeading randomTimeout, (message) =>
            @.eventsLoadUserstories(message)
        @events.subscribe @scope, routingKeyUserstories, (message) =>
            onUserstoriesMessage(message)
            @scope.$broadcast("kanban:realtime:userstories", message)

        # For project attributes (swimlanes, statuses,...) events
        routingKeyProject = "changes.project.#{@scope.projectId}.projects"
        onProjectMessage = debounceLeading randomTimeout, (message) =>
            if message.matches in [
                ...
                if @.isLightboxOpened
                    ...
                    @.refreshAfterSwimlanesOrUserstoryStatusesHaveChanged()
        @events.subscribe @scope, routingKeyProject, (message) =>
            onProjectMessage(message)
            @scope.$broadcast("kanban:realtime:projects", message)
...
# The view-layer factories below are intentionally unregistered: the board's
# markup is rendered by the React tree, and these bodies are kept as the
# authoritative reference for the behaviour it has to reproduce -- the WIP-limit
# arithmetic, the write-once virtualisation latch, the archived-column lifecycle,
# the sticky-header geometry and the drag-hover auto-open.
