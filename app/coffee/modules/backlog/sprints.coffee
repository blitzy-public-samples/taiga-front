...
## Every factory in this file is intentionally unregistered: the sprint sidebar and
## its cards are rendered by React, and these bodies are the authoritative reference
## for the behaviour it has to reproduce. The parts most easily lost: the
## `sprintform:edit` broadcast is what opens the retained sprint lightbox; the
## closed-sprints toggle depends on the `backlog:load-closed-sprints`,
## `backlog:unload-closed-sprints` and `closed-sprints:reloaded` names surviving
## verbatim; and one open/closed state is expressed by two different classes on two
## different elements -- `active` on the compact sprint and `open` on the sprint
## table -- so both have to be emitted for the existing stylesheet to apply.
##
## The dead local, the factory-scoped flag shared across instances, the injected
## parameter that is always undefined and the fat arrow that raises a coffeelint
## warning are all pre-existing and preserved deliberately, so this reference stays
