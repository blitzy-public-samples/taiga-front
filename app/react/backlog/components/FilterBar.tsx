...
/**
 * FilterBar (Backlog) — React 18 port of the AngularJS `tg-filter` sidebar
 * component, hosted inside the Backlog screen's `#backlog-filter` sidebar.
 *
 * This is a faithful, standalone reproduction of the legacy `tg-filter` component
 * (its template `filter.jade` for the DOM, and its controller
 * `filter.controller.coffee` — `FilterController` aliased `vm` — for behaviour).
 * It resolves QA finding BL-6: the Backlog previously rendered an EMPTY
 * `<div id="backlog-filter">` placeholder, so only the free-text `q` search
 * worked and the categorical/custom filter dimension was unreachable. The
 * AngularJS baseline (`backlog.jade:126-140`) hosted a full `tg-filter` with
 * category lists, custom filters, selected chips, and the add/remove/save
 * handlers — this component restores that behaviour with full parity.
 *
 * WHY BACKLOG-LOCAL (coexistence boundary — AAP 0.7 HARD RULES): the sibling
 * Kanban screen ships an equivalent `app/react/kanban/components/FilterBar.tsx`,
 * but the two feature areas are isolated by contract — the Backlog source tree
 * MUST NOT import from `../kanban/**`. This component is therefore a deliberate,
 * self-contained backlog copy. It DOES reuse the shared, framework-neutral
 * helpers under `../../shared/**` (translation, emoji, avatar), which is the
 * sanctioned cross-cutting layer both screens share.
 *
 * Design intent: ZERO visual change. The component reproduces the exact tag
 * names, CSS class names, and end-to-end (`e2e-*`) hooks used by the AngularJS
 * template so the existing compiled stylesheets (whose `tg-filter` selector
 * targets this root) render the React output identically. The root element is a
 * `<tg-filter>` custom element wrapping the two original top-level sections
 * (`.custom-filters` and `.filters-step-cat`). No stylesheet or asset is
 * imported here.
 *
 * Integration boundary (globals-only): FilterBar is a pure controlled view. All
 * filter data (`filters`, `customFilters`, `selectedFilters`) and every mutation
 * flow through props to the owner (`BacklogApp` / `useBacklog`), which performs
 * the `/api/v1/` calls. FilterBar imports NOTHING from the AngularJS /
 * CoffeeScript codebase and pulls in no third-party runtime dependency beyond
 * React itself.
 *
 * REFERENCE-ONLY sources (reproduced, never imported): the legacy filter
 * component's template (`filter.jade`), controller (`filter.controller.coffee`),
 * and directive (`filter.directive.coffee`), plus the backlog route template
 * (`backlog.jade`). These live in the legacy client tree and are intentionally
 * NOT imported.
 *
 * Accessibility vs. visual fidelity: the AAP mandates "zero visual change"
 * (Section 0.3.4) and places the frozen visual/design contract ABOVE
 * accessibility heuristics (Discovery precedence rule D1). The inherited Taiga
 * color tokens that drive this component (e.g. the `#008AA8` link/brand color)
 * are therefore NOT altered here. All accessibility work in this file is
 * STRUCTURAL only — semantic roles, accessible names, keyboard operability, and
 * a focus ring that REUSES the existing brand color — and introduces no new
 * color, spacing, or layout value.
 */
...
// Shared, AngularJS-free translation layer. Every title, mode label, control, and
// error message resolves through the active Taiga locale catalog — exactly as the
// legacy `translate` filter / service did in filter.jade.
import { t } from '../../shared/i18n';
// Emoji-shortcode rendering for applied TAG filters, reproducing the legacy
...
// Avatar resolution for user filter options, reproducing the `tg-avatar`
...
/**
 * Visually-hidden-but-focusable style for the include/exclude radio inputs.
 *
 * The reference SCSS hid the radio with `.custom-radio input[type=radio] { display: none }`,
 * which removes it from the tab order entirely and leaves the focusable-but-inert
 * `label.filter-mode[tabindex=0]` with no key handling. We instead keep the NATIVE radio in
 * the accessibility/tab tree — overriding `display:none` with `display:block` and shrinking
 * it to a clipped 1px box — so screen readers announce a real radio group and keyboard users
 * get native arrow-key selection and Space activation for free. The clip/absolute technique
 * removes it from the visual flow, so the rendered layout is byte-identical to the legacy
 * (ZERO visual change).
 */
 ...
 * hidden) native radio receives keyboard focus (WCAG 2.4.7). Reuses the existing
 * Taiga link/brand color `#008AA8` (no NEW color is introduced). `outline`
 * occupies no layout space and `outlineOffset` lifts the ring clear of the 16px
 * mark, so the affordance is purely additive with no layout shift.
 */
...
/** A collapsible filter category (e.g. status, tags, assigned_users, owner, role). */
export interface FilterCategory {
  dataType: string; // 'status' | 'tags' | 'assigned_users' | 'owner' | 'role' | ...
  title: string;
...
/** A filter currently applied to the backlog, in include or exclude mode. */
...
export interface FilterBarProps {
  ...
  /** Categories whose `dataType` appears here are hidden. Backlog passes `[]`. Default `[]`. */
  ...
  /**
   * API-complete: the header query search (`tg-input-search`) is rendered by
   * BacklogApp in `.backlog-table-options-start`, NOT inside this sidebar.
   * Exposed for parity only; the sidebar DOM below never wires it. Mirrors
   * `onChangeQ: "&"` in filter.directive.coffee, which is likewise declared but
   * unreferenced by filter.jade.
   */
 ...
 *
 * NOTE: The AngularJS directive `link` (filter.directive.coffee) ran a
 * `ResizeObserver` on `.js-taskboard-manager` to clamp `--filter-list-max-height`
 * (100..380) and toggled an `open` class from an `open` attribute. Both are
 * TASKBOARD-only: the backlog route has no `.js-taskboard-manager` element (it
 * uses `.backlog-manager`) and passes no `open` attribute (see backlog.jade), so
 * the observer never fires and the SCSS default `--filter-list-max-height: 380px`
 * applies. The link is therefore intentionally OMITTED here — it is inert on the
 * backlog route.
 */
const FilterBar = (props: FilterBarProps): JSX.Element => {
  ...
  // Which include/exclude radio currently holds keyboard focus (focus ring).
  ...
  // Categories to render — a category whose `dataType` is in `excludeFilters` is hidden.
   ...
   * equivalent and `emojify` HTML-escapes first, so the sink is safe), while every
   * other data type renders the plain `{{it.name}}` text node. The icon-only remove
   * button gains an accessible name.
   */
  const renderAppliedChip = (it: AppliedFilter): JSX.Element => (
    <div
      key={it.key ?? `${it.dataType}-${it.id}`}
      className={`single-applied-filter ng-animate-disabled ${it.mode}`}
    >
      {it.dataType === 'tags' ? (
        <div className="name" dangerouslySetInnerHTML={{ __html: emojify(it.name, emojiMap) }} />
      ) : (
    ...
    <TgFilter>
      ...
      <div className="custom-filters">
        ...
        {customFilterForm && selectedFilters.length > 0 && (
          ...
          >
            {/*
             * a11y: add an `id`/`name` (alongside the existing `aria-label`) so the
             * browser no longer logs "A form field element should have an id or name
             * attribute". No visible `<label>` is added — the accessible name is
             * supplied by `aria-label`, matching the legacy filter markup so visual
             * parity is preserved.
             */}
            <input
              className={
              ...
              id="backlog-add-filter-name"
              name="backlog-add-filter-name"
      ...
      <div className="filters-step-cat">
          ...
          <div className="filters-applied">
            {includedFilters.length > 0 && (
              <div className="filters-included">
                <div className="filters-title">{t('COMMON.FILTERS.ADVANCED_FILTERS.INCLUDED')}</div>
                <div className="filters-wrapper">{includedFilters.map(renderAppliedChip)}</div>
              </div>
            )}
            {excludedFilters.length > 0 && (
              <div className="filters-excluded">
                <div className="filters-title">{t('COMMON.FILTERS.ADVANCED_FILTERS.EXCLUDED')}</div>
                <div className="filters-wrapper">{excludedFilters.map(renderAppliedChip)}</div>
              </div>
            )}
          </div>
        )}
        ...
        <div className="filters-advanced">
          {/*
            The include/exclude mode selector is a semantic radio group. The legacy
            markup hid the native radio (`display:none`) and made the
            `<label tabindex=0>` focusable but WITHOUT any key handler. We instead
            keep the NATIVE radio focusable-but-visually-hidden (see
            VISUALLY_HIDDEN_INPUT) inside an explicit `role="radiogroup"`, so screen
            readers announce a real group and keyboard users get native arrow-key
            selection + Space activation. The label's `tabindex` is removed (the
            input is now the single focusable control). The visible selection is
            still driven by the `.active` class exactly as before, so there is ZERO
            visual change.
          */}
          <div
            ...
            {filterModeOptions.map((option) => (
              <div key={option} className="custom-radio">
                <input
                  ...
                  onBlur={() => setFocusedMode((current) => (current === option ? null : current))}
                />
                <label
                  className={`filter-mode ${option}` + (filterMode === option ? ' active' : '')}
                  htmlFor={`filter-mode-${option}`}
                >
                  <div className="radio-mark" style={focusedMode === option ? FOCUS_OUTLINE : undefined}>
            ...
            {visibleFilters
              ...
              .map((filter) => (
                <li key={filter.dataType} className={isOpen(filter.dataType) ? 'selected' : ''}>
                  <button
                    className={
                      'filters-cat-single e2e-category' + (isOpen(filter.dataType) ? ' selected' : '')
                    }
                  ...
                  {/*
                    Behavioral-equivalent of `tg-filter-slide-down`. That legacy
                    directive is effectively INERT: its `open()` merely hides
                    `.filter-list` and then calls `.show()` on a bare text node, with
                    no CSS transition, duration, or easing. It therefore produces no
                    observable animation and leaves an identical resting state.
                    Conditionally mounting the list when the category is open
                    reproduces that exact end state (present when expanded, absent when
                    collapsed). The list keeps its `id` so the category toggle's
                    ...
                    <div className="filter-list" id={`filter-list-${filter.dataType}`}>
                      {filter.content
                        ...
                        .map((it) => {
                          ...
                          // Resolve the option's avatar exactly as the legacy `tg-avatar`
                          // directive did (photo / gravatar / deterministic local
                          // placeholder). Computed only for user categories.
                          ...
                          return (
                            ...
                            >
                              {/*
                                Reproduce the `tg-avatar` directive's DOM contract exactly —
                                it set `src=avatar.url`, `title`/`alt` to the interpolated
                                `"#{avatar.fullName}"` (String coercion), and the element
                                background to `avatar.bg || ''`. This renders a REAL image
                                (photo, gravatar, or deterministic local placeholder) rather
                                than `<img src="">`.
                              */}
                              {isUser && avatar && (
                                <img
