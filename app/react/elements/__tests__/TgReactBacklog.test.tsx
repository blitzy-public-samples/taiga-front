...
/**
 * Unit spec for the TgReactBacklog custom-element wrapper.
 *
 * Verifies the AngularJS <-> React coexistence bridge:
 *  - connectedCallback mounts <BacklogApp> (React root created over the host element)
 *  - the project slug is resolved from the `project-slug` attribute, falling back
 *    to parsing window.location.pathname (/project/:pslug/backlog, app.coffee:226)
 ...
 * no network — BacklogApp is mocked at the module boundary. Behavioral origin of
 * the mounted screen: BacklogController [backlog/main.coffee:715] (reference only;
 * this spec tests the React wrapper, never the CoffeeScript).
 *
 * This spec is the direct mirror of TgReactKanban.test.tsx (same structure; only
...
// module (app/react/backlog/BacklogApp) that TgReactBacklog imports via
// '../backlog/BacklogApp', so the wrapper renders this lightweight marker instead
// of the real (heavy) screen — none of BacklogApp's deps (hooks, reducers,
// shared/api, @dnd-kit) load or run. The marker echoes the resolved projectSlug
// into a data attribute for assertion, and exports BOTH the named `BacklogApp` and
// a `default` so it intercepts regardless of which specifier the wrapper uses.
jest.mock('../../backlog/BacklogApp', () => {
  const BacklogAppMock = ({ projectSlug }: { projectSlug: string }) => (
    <div data-testid="backlog-app" data-slug={projectSlug} />
  );
  return { __esModule: true, BacklogApp: BacklogAppMock, default: BacklogAppMock };
});

import { TgReactBacklog } from '../TgReactBacklog';

const TAG = 'tg-react-backlog';
...
if (!customElements.get(TAG)) {
  customElements.define(TAG, TgReactBacklog);
}

describe('TgReactBacklog custom element', () => {
  let el: TgReactBacklog;

  beforeEach(() => {
    jest.clearAllMocks();
    // Deterministic default location with no `project` segment => slug resolves to ''.
    window.history.pushState({}, '', '/');
    el = document.createElement(TAG) as TgReactBacklog;
  });
  ...
  it('mounts BacklogApp into the host element on connect', async () => {
    ...
    await waitFor(() => {
      expect(el.querySelector('[data-testid="backlog-app"]')).not.toBeNull();
    });
  });

  it('passes the project-slug attribute value to BacklogApp', async () => {
    el.setAttribute('project-slug', 'test-slug');
  ...
  it('falls back to parsing the URL when no attribute is set', async () => {
    window.history.pushState({}, '', '/project/foo/backlog');
  ...
  it('does not remount when connectedCallback fires again while already connected', async () => {
    ...
    await waitFor(() => {
      expect(el.querySelectorAll('[data-testid="backlog-app"]').length).toBe(1);
    });
  ...
  it('unmounts the React tree on disconnect', async () => {
    ...
    expect(el.querySelector('[data-testid="backlog-app"]')).toBeNull();
