 ...
 * TgReactBacklog
 * --------------
 * The React <-> AngularJS *coexistence boundary* for the Backlog /
 * sprint-planning screen. This custom-element class is the thin seam that lets
 * a React 18 tree live inside the AngularJS 1.5.10 host DOM without either
 * framework knowing about the other's internals (Blitzy AAP 0.1-0.6).
 *
 * WHAT IT DOES
 ...
 * everything effectful lives inside `BacklogApp` and `../shared/*`. This wrapper
 * only resolves the project slug from the preserved route and manages the React
 * mount/unmount lifecycle.
 *
 * WHAT IT REPLACES
 *   It supersedes the ROUTE-LEVEL rendering of the AngularJS `BacklogController`
 *   (`app/coffee/modules/backlog/main.coffee:715`,
 *   `module.controller("BacklogController", BacklogController)`). The template
 *   `app/partials/backlog/backlog.jade` hosts `<tg-react-backlog>` instead of the
 *   legacy controller markup (that swap is another agent's change). The AngularJS
 *   `taigaBacklog` module registration is intentionally RETAINED and is never
 *   referenced here.
 *
 * HOW IT IS REGISTERED (verified — do NOT re-derive)
 *   The entry point `app/react/index.tsx` performs a NAMED import of this class
 *   (`import { TgReactBacklog } from './elements/TgReactBacklog';`) and registers
 *   the tag via `customElements.define('tg-react-backlog', TgReactBacklog)`. That
 ...
 *   `<tg-react-backlog>` (in the `backlog.jade` route template), AngularJS treats
...
// The React feature component this wrapper mounts. `BacklogApp` is a NAMED export
// of `../backlog/BacklogApp` (it also ships a default). It receives the project
 ...
 *
 * The AngularJS controller reads `@params.pslug`, but this React wrapper is
 * framework-agnostic and must not depend on AngularJS `$routeParams`. AngularJS
 * runs in HTML5-mode routing (`app/coffee/app.coffee`), so the live path is in
 * `window.location.pathname`, shaped `/project/:pslug/backlog`
 * (`app/coffee/app.coffee:226`).
 *
 ...
 * downstream `BacklogApp`/`useBacklog` layer decides how to handle an empty slug.
 */
 ...
 * Backlog screen. Registered as `<tg-react-backlog>` by `app/react/index.tsx`
 * (this class never calls `customElements.define` itself).
 */
export class TgReactBacklog extends HTMLElement {
  /**
   ...
   * for `<tg-react-backlog>`, happens when AngularJS enters the
   * `/project/:pslug/backlog` route and `ng-view` inserts `backlog.jade`.
   *
   * Creates a React root over the host element and renders <BacklogApp>, passing
  ...
  connectedCallback(): void {
    ...
    // `createElement`. Equivalent to `<BacklogApp projectSlug={projectSlug} />`
    // in a `.tsx` file. Only `{ projectSlug }` is supplied; the optional
    // `projectId` is resolved internally by `BacklogApp`/`useBacklog`.
    this.root.render(createElement(BacklogApp, { projectSlug }));
  }

  /**
   * Invoked by the browser when the element is detached from the document —
   * which, for `<tg-react-backlog>`, happens when AngularJS leaves the backlog
...
// the NAMED export (`import { TgReactBacklog }`); the default is provided for
// symmetry with the sibling `TgReactKanban.ts` and for flexible consumption.
export default TgReactBacklog;
